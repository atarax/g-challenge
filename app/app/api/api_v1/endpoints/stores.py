...
def create_store(
    *, db: Session = Depends(deps.get_db), store_in: schemas.StoreCreate,
) -> Any:
    """
    Create new store.
    """
...
def update_store(
    *, db: Session = Depends(deps.get_db), id: int, store_in: schemas.StoreUpdate,
) -> Any:
    """
    Update a store.
    """
...
def read_store(*, db: Session = Depends(deps.get_db), id: int,) -> Any:
    """
    Get store by ID.
    """
    store = crud.store.get(db=db, id=id)
...
def delete_store(*, db: Session = Depends(deps.get_db), id: int,) -> Any:
    """
    Delete a store.
    """
    store = crud.store.get(db=db, id=id)
